...
describeWithResources('Aggregate', ({eventStore}) => {
  test('static commit() - no keySchema', async () => {
    ...
    const a = TestAggregate.commit([{type: 'SomeEvent', properties: {myProperty: 'a'}}])
    const b = TestAggregate.commit([{type: 'SomeEvent', properties: {myProperty: 'b'}}])
...
describeWithResources('Aggregate', ({eventStore}) => {
  test('static commit() - with keySchema', async () => {
    ...
    const a = TestAggregate.commit('myid', [{type: 'SomeEvent', properties: {myProperty: 'a'}}])
    const b = TestAggregate.commit({id: 'myid'}, [{type: 'SomeEvent', properties: {myProperty: 'b'}}])

    const [commitA, commitB] = await Promise.all([a, b])
    const commitsInStore = await iterableToArray(
      TestAggregate.eventStore.queryAggregateCommits('TestAggregate', 'myid').commits
    )
    expect(commitsInStore).toContainEqual(commitA)
    expect(commitsInStore).toContainEqual(commitB)
  })
