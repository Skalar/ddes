...
describeWithResources('scenarios/store-transformations: commit copy-and-transformation', {stores: true}, context => {
  ...
  test(`modifying commit`, async () => {
    ...
    await expect(iterableToArray(source.scan().commits)).resolves.toMatchObject(testCommits)

    await expect(iterableToArray(target.scan().commits)).resolves.toMatchObject([
      {
        aggregateKey: 'key',
        aggregateType: 'AggregateA',
        aggregateVersion: 1,
