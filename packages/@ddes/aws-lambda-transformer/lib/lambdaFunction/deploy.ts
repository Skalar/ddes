 ...
 * @hidden
 */
export default async function deploy(id: string, transformationPath: string, config: LambdaTransformerConfig) {
  const {
    awsConfig,
  ...
  const uploadPromise = s3.upload({Bucket: id, Key: 'transformationFunction.zip', Body: uploadStream}).promise()
      ...
      try {
        await lambda
          .createFunction({
            ...
            Handler: 'node_modules/@ddes/aws-lambda-transformer/index.lambdaHandler',
        ...
        if (!(error.code === 'InvalidParameterValueException') || attempt >= 30) {
          throw error
        }
