import {AggregateCommit, EventStore, VersionConflictError, AggregateEvent} from '@ddes/core'
...
export class DynamodbEventStore extends EventStore {
  ...
  public async *chronologicalQuery<TAggregateCommit extends AggregateCommit>(params: {
    ...
    const maxDate =
      max instanceof Date ? max : new Date(max.replace(/^(\d{4})(\d{2})(\d{2}).*/, '$1-$2-$3'))
    const maxCursor = max instanceof Date ? max.toISOString().replace(/[^0-9]/g, '') + ';' : max

    const minDate =
      min instanceof Date ? min : new Date(min.replace(/^(\d{4})(\d{2})(\d{2}).*/, '$1-$2-$3'))
    ...
    for (const partition of chronologicalPartitionIterator({
      ...
      for await (const queryResult of store.request('query', queryParams)) {
        ...
        if (queryResult.Items?.length) {
          yield queryResult.Items?.map(i => this.unmarshallCommit<TAggregateCommit>(i))
        }
    ...
    for await (const instanceQueryResult of this.request('query', {
      ...
      if (instanceQueryResult.Items) {
        for (const rawItem of instanceQueryResult.Items) {
          ...
          for await (const commits of this.queryAggregateCommits<TAggregateCommit>(a, r)) {
            yield commits
          }
        }
      }
  ...
  protected marshallCommit(commit: AggregateCommit) {
    const {
      ...
      aggregateVersion,
      expiresAt,
      chronologicalPartition: chronologicalPartition = 'default',
      chronologicalKey,
    } = commit
  ...
  protected unmarshallCommit<TAggregateCommit extends AggregateCommit>(
    ...
    const commit = {
      ...
      chronologicalPartition: unmarshalled.p.substr(8, unmarshalled.p.length - 8),
  ...
  protected request(
    ...
    params: {filterExpressions?: string[]; limit?: number} & Partial<DynamoDB.ScanInput>
