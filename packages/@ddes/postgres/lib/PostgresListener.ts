...
export class PostgresListener extends EventEmitter {
	...
	protected connectionHealthCheckTimer?: NodeJS.Timeout
	...
	constructor(
		...
		this.connectionHealthCheckInterval =
			options.connectionHealthCheckInterval ?? 30
				...
				this.eventNames()
					...
					.map((eventName) =>
						this.connection?.query(
							sql`LISTEN ${sql.ident(eventName.toString())}`,
						),
					),
			)
		} catch (error) {
	...
	protected async verifyConnectionHealth() {
		if (!this.connection)
			throw new Error('Trying to verify connection health when no connection')
		...
		try {
			...
			this.emit(
				'connectionError',
				new Error(`Postgres connection health check failed: ${error}`),
			)
			this.handleDisconnect()
		}
	}
