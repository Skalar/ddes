import { AggregateCommit, AggregateEvent, EventStore } from './EventStore'
...
> {
	...
	async get(
		...
		const getCommitsOptions: Parameters<
			EventStore['queryAggregateCommits']
		>[2] = {}
		...
		if (this.config.snapshots && options.useSnapshots !== false) {
			const snapshot = await this.config.snapshots.read([
				this.config.type,
				keyString,
			])
			...
			if (snapshot) {
				...
				if (
					...
					this.config.snapshots.compatibilityVersion
				) {
					...
					snapshotIsUsable =
						(options.time instanceof Date
							...
							: options.time) >= snapshot.timestamp
				} else {
		...
		if (
			...
			await this.config.snapshots.write(
				[this.config.type, keyString],
				snapshotData,
			)
		}
	...
	async commit(
		...
		const commit: AggregateCommit<TEvent, TAggregateType> = this.build(
			key,
			version,
			events,
		)

		return await this.config.store.commit<
			AggregateCommit<TEvent, TAggregateType>
		>(commit)
	}
				...
				if (key && key !== commit.aggregateKey) {
					if (state)
						yield {
							...
							timestamp,
							state,
						} as AggregateInstanceData<TState>
					state = undefined
				}
	...
	public propsFromKey(key: string): TKeyProps {
		if (!this.config.keyPropSeparator) throw new Error('No keyPropSeparator')
