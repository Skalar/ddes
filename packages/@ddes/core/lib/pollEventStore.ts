...
export async function* pollEventStore<
	...
	TAggregateCommitMap extends Record<
		string,
		any
	> = ConstructAggregateCommitType<TAggregates>,
	TAggregateCommit extends
		AggregateCommit = TAggregateCommitMap[keyof TAggregateCommitMap],
>(
	params: {
		store: EventStore
	...
	const poller = pollWithBackoff(
		...
		() =>
			store.chronologicalQuery<TAggregateCommit>({
				...
				exclusiveMin: true,
				...(aggregateRoots && {
					aggregateTypes: Object.values(aggregateRoots).map(
						(v) => v.config.type,
					),
				}),
				chronologicalPartition,
