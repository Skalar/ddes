...
>(
	...
	const consumeBatch = () => {
		let progressCursor: string | undefined

		if (backlog.size) {
			// We have commits in the backlog, which means new cursor cannot be higher than theirs
			const earliestCursorInBacklog =
				Array.from(backlog).shift()?.chronologicalKey || ''

			for (const commitInBatch of batch) {
				if (commitInBatch.chronologicalKey >= earliestCursorInBacklog) {
					if (
						!postponedCursor ||
						commitInBatch.chronologicalKey > postponedCursor
					) {
